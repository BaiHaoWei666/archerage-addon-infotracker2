...
- 角色資訊與副本由共用資料層按需輪詢：角色資訊每秒、副本每五秒；兩視窗共用每項結果與副本清單，只查詢目前顯示分類需要的項目，切入時補查過期資料。角色資訊的每日任務解鎖狀態直接共用挑戰七格資料。
- 收入由事件累計並立即存檔，下一幀合併通知兩視窗；顯示收入頁時每秒共用一次跨日檢查，隱藏時於下次入帳或顯示才檢查。資料未變不通知畫面；內容未變不寫文字，結構未變不重排。視窗不再各自定時查詢資料，滑鼠懸停、勾選同步與延遲操作仍使用 UI 計時。
